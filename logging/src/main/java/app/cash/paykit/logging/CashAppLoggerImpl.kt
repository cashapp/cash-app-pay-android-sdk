...
class CashAppLoggerImpl : CashAppLogger {
  ...
  override fun retrieveLogs(): List<CashAppLogEntry> = history.retrieveLogs()

  override fun logsAsString(): String = buildString {
  ...
  private fun logLevelToString(level: Int): String = when (level) {
    Log.VERBOSE -> "VERBOSE"
    Log.DEBUG -> "DEBUG"
    Log.INFO -> "INFO"
    Log.WARN -> "WARN"
